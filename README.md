# Site Agent — Proof your work. Prevent disputes. Get paid.
...
1. **Connect** your repo to Vercel (root directory is the project root).
...
- **Team size limits** — Pro and Team plan member limits are in `src/lib/team-limits.ts`. Edit `TEAM_PLAN_MAX_SLOTS` and `PRO_PLAN_MAX_SLOTS`. After changing, update the pricing page and team page copy (see comments in that file).
- **Member activity** — Pro/Team owners see a "Recent activity" feed on the Team page showing who created jobs, added photos, updated notes, captured signatures, and generated reports.
